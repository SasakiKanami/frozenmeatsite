...
function handleGuestCheckout(event) {
    ...
    if (checkoutCard && confirmationCard) {
        ...
        // Push this order + stock deduction to the Admin Panel's records
        logOrderForAdmin(orderRef, guestName, guestPhone, fulfillmentType, paymentMethod, cart, document.getElementById('cart-total-price').textContent);
        deductInventoryFIFO(cart);
...
// 8. Save the completed order so the Admin Panel can list it under Orders
function logOrderForAdmin(orderRef, guestName, guestPhone, fulfillmentType, paymentMethod, orderItems, totalText) {
    const orders = JSON.parse(localStorage.getItem('carni_orders')) || [];
    orders.unshift({
        ref: orderRef,
        name: guestName,
        phone: guestPhone,
    ...
    localStorage.setItem('carni_orders', JSON.stringify(orders));
}

// 9. Mimic the proposed "Automated FIFO Inventory" module by decrementing
//    matching stock the moment a guest order is confirmed
function deductInventoryFIFO(orderItems) {
    const inventory = JSON.parse(localStorage.getItem('carni_inventory'));
    if (!inventory) return; // Admin Panel hasn't initialized stock yet

    orderItems.forEach(cartItem => {
        const stockItem = inventory.find(inv => cartItem.name.includes(inv.name));
        if (stockItem) {
